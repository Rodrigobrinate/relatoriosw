...
def _normalize_interface_name(name: str) -> str:
    name = name.strip()
    # Converte "HundredGigE0/0/1" ou "100GE 0/0/1" para "100GE0/0/1"
    name = re.sub(r"^(HundredGigE|100GE)\s*(\d+.*)", r"100GE\2", name)
    # Converte "FortyGigE0/0/1" ou "40GE 0/0/1" para "40GE0/0/1"
    name = re.sub(r"^(FortyGigE|40GE)\s*(\d+.*)", r"40GE\2", name)
    # Converte "XGigabitEthernet0/0/1" ou "10GE 0/0/1" para "XGE0/0/1"
    name = re.sub(r"^(XGigabitEthernet|TenGigE|10GE)\s*(\d+.*)", r"XGE\2", name)
    # Converte "GigabitEthernet0/0/1" ou "GE 0/0/1" para "GE0/0/1"
    name = re.sub(r"^(GigabitEthernet|GE)\s*(\d+.*)", r"GE\2", name)
    return name
...
def _parse_single_interface_block(output_text: str) -> dict | None:
    ...
    def get_float(key_regex):
        ...
        try:
            # Pega o primeiro número (int ou float) que encontrar
            val_match = re.search(r"(-?[\d.]+)", match.group(1))
            return float(val_match.group(1)) if val_match else None
        except (ValueError, TypeError, AttributeError):
            return None
            
    def get_first_float_from_multilane(key_regex, stop_keyword):
        """
        Extrai dados que podem ser multi-lane (como 100GE) e retorna
        APENAS O PRIMEIRO NÚMERO FLOAT encontrado.
        """
        regex = rf"^\s+{key_regex}\s*:\s*([\s\S]*?)(?=^\s+{stop_keyword}|^\s+-{{3,}})"
        match = re.search(regex, output_text, re.MULTILINE)
        if not match: return None
        try:
            # Pega o primeiro número (int or float) que encontrar no bloco
            val_match = re.search(r"(-?[\d.]+)", match.group(1))
            return float(val_match.group(1)) if val_match else None
        except (ValueError, TypeError, AttributeError):
            return None
    ...
    if reading_data['transceiver_status'] == "present":
        ...
        # --- INÍCIO DA MUDANÇA (Multi-lane) ---
        # Agora usamos get_first_float_from_multilane para obter um Float
        reading_data['bias_current'] = get_first_float_from_multilane(r"Bias Current\(mA\)", r"Bias High Threshold")
        reading_data['rx_power'] = get_first_float_from_multilane(r"RX Power\(dBM\)", r"RX Power High Warning")
        reading_data['tx_power'] = get_first_float_from_multilane(r"TX Power\(dBM\)", r"TX Power High Warning")
        # --- FIM DA MUDANÇA ---
...
def parse_global_verbose_output(global_output_text: str) -> dict:
    ...
    interface_header_regex = r"^([A-Za-z-]+\d+[\d/.]+) transceiver information:"
